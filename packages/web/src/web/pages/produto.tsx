...
export default function Produto() {
  ...
  const blocks: Record<string, Block> = {
    ...
    videos: {
      ...
      render: (tone) => <ProductVideos productName={product.name} videos={videos} tone={tone} />,
    },
    anatomia: {
      show: isBelt,
      render: () => (
    ...
    modelos: {
      show: product.models.length > 0 && specKeys.length > 0,
      render: (tone) => (
        <Section tone={tone}>
          <Reveal>
    ...
    material: {
      show: !!process || hasMaterialFit(product.slug),
      render: (tone) => (
        process ? (
          <ProductProcess process={process} tone={tone} />
        ) : (
          <Section tone={tone}>
            <Reveal>
    ...
    cta: { render: () => <CtaBand waMessage={waMessage} /> },
  };
  ...
  return (
    <>
      ...
      <PageSections page="produto" blocks={blocks} />
    </>
  );
}
